...
At the moment, we still rely on `--loa`d because downstream steps need access to the built image locally for validation. However, a cleaner approach is available: run the remote builder within the same network as the registry or deployment environment and switch to --push:
...
Using a remote BuildKit daemon with persistent caching provides significant and repeatable performance improvements for dependency-heavy builds — especially for multi-stage builds with shared intermediate artifacts. The improvement is not theoretical: cold builds become warm builds, and minutes become seconds.
...
By moving the builder out of the CI runner — instead of trying to push cache into the runner — we unlock the full caching model that BuildKit was designed to provide.

One important remark, this solution does not require Kubernetes.
While this post demonstrated a BuildKit daemon running in a Kubernetes cluster backed by a Persistent Volume Claim, the same setup can run: on a VM, on a home lab server, on a NAS, on a $5/month cloud instance or even on your laptop. The only requirement is that the CI runner can reach the BuildKit daemon over a secure channel.
