...
The solution is clear — we need persistent builders.
We deploy a remote BuildKit daemon with persistent storage in our Kubernetes cluster, ensuring that cache data survives across builds and can be reused by all CI jobs.
...
> Notice that a StatefulSet would be a better fit for this use case, but for simplicity we used a Deployment with a single replica. The buildkit deployment+service example served as the main inspiration for this setup. You can find it [here](https://github.com/moby/buildkit/tree/master/examples/kubernetes).
...
We have saved those four values as CI/CD variables/secrets in our GitLab project so we can run something like the snippet below:
...
While this post demonstrates a BuildKit daemon running in a Kubernetes cluster backed by a Persistent Volume Claim, the same setup can run on a VM, on a $5/month cloud instance, or even on your laptop. The only requirement is that the CI runner can reach the BuildKit daemon over a secure channel.
This can be achieved using Cloudflare Tunnel, Tailscale / WireGuard, or a simple testing setup with ngrok.
