...
This architecture scales beautifully but introduces one major performance bottleneck: loss of internal build cache.
...
Yet, it still can’t accelerate the internal BuildKit cache system that Docker relies on for cache mount reuse.
...
> Notice that, an StatefulSet would be a better fit for this use case, but for simplicity we used a Deployment with a single replica.
